...
- **补齐缺失的 CAPTCHA 蓝图资源（新）**：
  - 将 `benchmark/apis/{arkose, lemin, tencent, yandex}` 目录连同模板、静态文件与 `challenges.json` 正式纳入仓库，CI checkout 后即可加载全部蓝图。
  - 移除历史遗留的 `.DS_Store` 缓存并添加 `.gitignore` 规则，确保未来不会再遗漏关键资源或混入系统文件。
  - 修复后 `test_captcha_endpoints_http` 能对 26 条样本全部获得 200，Playwright 集成用例不再因 404 被迫跳过。
